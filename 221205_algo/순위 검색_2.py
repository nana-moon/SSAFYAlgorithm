...
def lower_bound(start, end, list, target):
    if start > end:
        return 30
    ...
    if list[mid] == target:
        ...
        lower_bound(start, mid-1, list, target)
    else:
        lower_bound(mid+1, end, list, target)
...
def solution(infos, querys):
    ...
    for query in querys:
        ...
        if target_key in dic.keys():
            answer.append(len(dic[target_key])-lower_bound(0, len(dic[target_key]),dic[target_key], target))
        else:
            answer.append(0)
